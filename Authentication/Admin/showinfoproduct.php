<?php
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $response .= '<img class="mt-3" width="100px" src="../../Public/img/'.$image.'">';
        $response .= '</div>';
        $response .= '<div class="form-group">';
        $response .= '<label for="mota">Mô Tả</label>';
        $response .= '<label class="text-warning ml-2" id="ermota"> </label>';
        $response .= '<textarea id="mota"  cols="90" rows="5">' . $describe . '</textarea>';
        $response .= '</div>';
        $response .= '<div class="form-group">';
        ...
        $response .= '<a class="btn btn-primary text-white mb-3 mr-5" href="../Admin/Adminproduct.php">Trở Về</a>';
